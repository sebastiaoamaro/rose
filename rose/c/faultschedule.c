...
char* get_veth_interface_name_docker(const char* container_name);
char* get_veth_interface_name_lxc(const char* container_name);
...
char* get_veth_interface_name_lxc(const char* container_name) {
    ...
    // Get the iflink of the container's eth0 interface
    snprintf(command, sizeof(command), "lxc exec %s -- bash -c 'cat /sys/class/net/eth0/iflink'", container_name);
    FILE* pipe = popen(command, "r");
    if (!pipe) {
        perror("popen");
        return NULL;
...
execution_plan* build_execution_plan(){
    ...
    create_execution_plan(exe_plan,"",0,"","",30,0,0);
    return exe_plan;
}
 tracer* build_tracer(){
...
node* build_nodes(){
    ...
    create_node(&nodes[0],"zookeeper",0,"","","","/home/sebastiaoamaro/phd/Anduril-test/Anduril/ground_truth/zookeeper-4203/run-original-test.sh","",0,1,"","",0);
...
fault* build_faults_extra(){
    ...
    syscall0.syscall = ACCEPT_FAULT;
    syscall0.success = 0;
    syscall0.return_value = -5;
    fault_details0.syscall = syscall0;
    create_fault(&faults[0],"syscall1101",0,0,ACCEPT_FAULT,2,fault_details0,0,0,0,1,0);

    fault_condition fault_condition_0_0;
    systemcall syscall_0_0;
    fault_condition_0_0.type = SYSCALL;
    build_syscall(&syscall_0_0,ACCEPT_STATE,14);
