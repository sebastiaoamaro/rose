...
static int handle_event(void *ctx, void *data, size_t data_sz)
{
	...
	printf("interface: %s\tprotocol: %s\t %s:%d(src) -> %s:%d(dst) bytes: %u \n",
		ifname,
...
{
	exiting = true;
	printf("Sock thread finished \n");
}

int trace_connections()
{
	...
	while (!exiting) {
		...
		struct pair key, prev_key={};
		int fd = bpf_map__fd(skel->maps.network);
		__u32 value;
		int res;

		while(bpf_map_get_next_key(fd, &prev_key, &key) == 0) {
				printf("%s(src) %s(dst) \n", inet_ntoa((struct in_addr){key.src_addr}),inet_ntoa((struct in_addr){key.dst_addr}));
	...
	printf("Finished trace connections \n");

cleanup:
	close(bpf_map__fd(skel->maps.network));
