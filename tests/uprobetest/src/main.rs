...
fn main() -> Result<()>{
    ...
    let binary_path = "/home/sebastiaoamaro/phd/torefidevel/tests/uprobetest/src/write".to_string().clone();

    let function_name = "my_test_function".to_string().clone();


    println!("function name is {} binary_location is {} and symbol location is {} and pid is {}",function_name,binary_path.clone(),0,pid as i32);

    let opts = UprobeOpts{func_name:function_name.clone(),..Default::default()};
    ...
    let opts_ret = UprobeOpts{func_name:function_name.clone(),retprobe:true,..Default::default()};

    let uprobe_ret = skel.progs_mut().handle_uprobe_ret().attach_uprobe_with_opts(
        pid as i32,
        binary_path.clone(),
        0,
        opts_ret
