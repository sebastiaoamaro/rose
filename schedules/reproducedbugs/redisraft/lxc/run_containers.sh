...
for i in $(seq 1 $n); do
	...
	lxc file push /vagrant/schedules/reproducedbugs/redisraft/setup/redis_binaries/redis-cli18.04 n${i}/opt/redis/redis-cli -q
	lxc file push /vagrant/schedules/reproducedbugs/redisraft/setup/redis_binaries/redis-server18.04 n${i}/opt/redis/redis-server -q
	lxc file push /vagrant/schedules/reproducedbugs/redisraft/setup/redisraft_binaries/18.04redisraft$version.so n${i}/opt/redis/redisraft.so -q
	lxc file push /vagrant/schedules/reproducedbugs/redisraft/lxc/start_redis_node.sh n${i}/opt/redis/start_redis_node.sh -q
