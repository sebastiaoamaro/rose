    ...
    (let [id (node-id test node)]
      ...
      (let [e (:enable-server-auto-create-topics test)]
        (when-not (nil? e)
          ...
          (rpk! :config :set "redpanda.auto_create_topics_enabled" true)))
...
(defn wait-for-file-data [filepath]
  (:log-message "Waiting for file to have data...")
  (loop [f (java.io.File. filepath)]
    (if (pos? (.length f))
      (slurp f)  ;; Return file contents when ready
      (do
        (Thread/sleep 1000)
        (recur f)))))
...
(defrecord DB [node-ids tcpdump]
  ...
  (setup! [this test node]
    ...
    (wait-for-file-data "/tmp/signal_start_workload.txt"))
  ...
  (topic-partition-state [this node topic-partition]
    (topic-partition-state node topic-partition)))
