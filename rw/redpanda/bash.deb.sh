...
function die {
    ...
    echo -e "$prefix: "
    echo -e "$prefix: ${bold}You can contact Redpanda for further assistance.${reset}"
    echo -e "$prefix: "
    
    echo -e "$prefix: ${bold}URL: https://github.com/redpanda-data/redpanda/${reset}"
    echo -e "$prefix: "
...
function config_url {
    ...
    echo "https://dl.redpanda.com/public/redpanda/config.deb.txt?$params" | sed 's/ /%20/g'
}
...
EOF
 ...
 >>>>: doesn't work after an hour, please contact Redpanda
 >>>>: for assistance.
 >>>>:
EOF
        }
...
function import_gpg_key {
    local text="Importing 'redpanda/redpanda' repository GPG keys ..."
    echo_running "$text"

    local gpg_keyring_path="/usr/share/keyrings/redpanda-redpanda-archive-keyring.gpg"
    
    curl -1sLf "https://dl.redpanda.com/public/redpanda/gpg.988A7B0A4918BC85.key" | gpg --dearmor >> $gpg_keyring_path
    ...
    echo_okfail "Checking for apt signed-by key support ..." || {
        mv ${gpg_keyring_path} /etc/apt/trusted.gpg.d/redpanda-redpanda.gpg
    }
...
function setup_repository {
    local repo_path="/etc/apt/sources.list.d/redpanda-redpanda.list"
    check_fetch_config

    local text="Installing 'redpanda/redpanda' repository via apt ..."
    echo_running "$text"
...
echo "Executing the  setup script for the 'redpanda/redpanda' repository ..."
echo
