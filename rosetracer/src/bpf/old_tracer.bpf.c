...
//Outdated File with lots of eBPF programs
#include "vmlinux.h"
...
static inline int check_event_counter(){
	if (event_counter == 131072)
		event_counter = 0;
	return 0;
}
...
int handle_uprobe(struct pt_regs *ctx) {
		...
		bpf_map_update_elem(&events, &called_functions_counter, &key, BPF_ANY);
...
// 		bpf_printk("Called tcp_state_changed \n");
// }