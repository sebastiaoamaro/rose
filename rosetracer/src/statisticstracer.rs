...
pub fn run_tracing(
    ...
    if mode == "container" {
        trace_containers(
            ...
            functions.clone(),
            binary_path.clone(),
            &mut unattachable_probes,
        )
        .expect("Error occurrend tracing containers");
    } else {
        trace_processes(
            node_and_pid.clone(),
            &mut skel,
        ...
        .expect("Error occurrend tracing processes");
    }
    ...
    for probe_idx in probes_indexes_to_remove {
        write_to_file(
            "/tmp/uprobe_stats.txt".to_string(),
            format!("{}\n", functions.get(probe_idx as usize).unwrap()),
        )
        .expect("Failed to write to stats file");
    }
...
pub fn trace_containers(
    ...
    functions: Vec<String>,
    binary_path: String,
    unattachable_probes: &mut Vec<u64>,
) -> Result<()> {
...
pub fn start_tracing_process(
    ...
    hashmap_uprobes: &mut HashMap<i32, Vec<Link>>,
    functions: Vec<String>,
    binary_path: String,
    skel: &mut TracerSkel,
    unattachable_probes: &mut Vec<u64>,
