...
use nix::unistd::Pid;
use crate::auxiliary::{self, collect_fd_map, collect_network_delays, collect_network_info, monitor_pid, pin_maps, start_xdp_in_container};
...
pub fn run_tracing(
    ...
    let skel_builder = ProductionTracerSkelBuilder::default();
    ...
    if mode == "container"{
        trace_containers(nodes_info, &mut skel,&mut hashmap_pid_to_node, 
            &mut hashmap_links, functions.clone(), binary_path).expect("Failed to trace containers");
    }

    else if mode == "container_controlled" {
        trace_containers_controlled(nodes_info.clone(),&mut skel,&mut hashmap_pid_to_node,
            &mut hashmap_node_to_pid,&mut hashmap_links,functions.clone(),binary_path).expect("Failed to trace containers controlled");
        ...
        trace_processes(nodes_info, &mut skel,&mut hashmap_pid_to_node, 
            &mut hashmap_links, functions.clone(), binary_path).expect("Failed to trace processes");
        ...
        if network_device.len() > 0 {
            let if_index = if_nametoindex(network_device.as_str()).map_err(|e| e.to_string()).unwrap();
            _xdp_prog = skel.progs.xdp_pass.attach_xdp((if_index) as i32).expect("Failed to attach xdp");
        }
    }
        ...
        if network_device.len() > 0 {
            let if_index = if_nametoindex(network_device.as_str()).map_err(|e| e.to_string()).unwrap();
            _xdp_prog = skel_xdp.progs.xdp_pass.attach_xdp((if_index) as i32).expect("Failed to attach xdp");
        }

        trace_processes_controlled(nodes_info, &mut skel,&mut hashmap_pid_to_node,&mut hashmap_node_to_pid,
        &mut hashmap_links, functions.clone(), binary_path).expect("Failed to trace processes controlled");
...
pub fn trace_processes(nodes_info:String,skel:&mut ProductionTracerSkel,hashmap_pid_to_node:&mut HashMap<i32,String>,
    mut hashmap_links:&mut HashMap<i32,Vec<Link>>,functions: Vec<String>,binary_path:String)-> Result<()> {
    ...
    for line in reader.lines() {
        ...
        if parts.len() == 2 {
            ...
            start_tracing_process(pid, node_name.to_string(),&mut hashmap_links, functions.clone(), binary_path.clone(),skel,rx,&mut join_handles);
        } else {
...
pub fn start_tracing_process(pid:i32,container_name:String,hashmap_links:&mut HashMap<i32,Vec<Link>>,
    functions: Vec<String>,binary_path:String,skel:&mut ProductionTracerSkel,rx:Receiver<()>,join_handles:&mut Vec<JoinHandle<()>>){
...
pub fn trace_containers(nodes_info:String,skel:&mut ProductionTracerSkel,hashmap_pid_to_node:&mut HashMap<i32,String>,
    mut hashmap_links:&mut HashMap<i32,Vec<Link>>,functions: Vec<String>,binary_path:String)-> Result<()> {
    ...
    for line in reader.lines() {
        ...
        if parts.len() == 3 {
            ...
            let (tx, rx) = mpsc::channel();
            tx_handles.push(tx);
            start_tracing_container(pid, node_name.to_string(),&mut hashmap_links, 
            functions.clone(), binary_path.clone(),skel,rx,&mut join_handles);
...
pub fn start_tracing_container(pid:i32,container_name:String,hashmap_links:&mut HashMap<i32,Vec<Link>>,
    functions: Vec<String>,binary_path:String,skel:&mut ProductionTracerSkel,rx:Receiver<()>,join_handles:&mut Vec<JoinHandle<()>>){
    ...
    for (index_function, function) in functions.clone().iter().enumerate(){
        ...
        let uprobe = skel.progs.handle_uprobe.attach_uprobe_with_opts(pid, binary_location.clone(), 0, opts);
...
pub fn trace_containers_controlled(nodes_info:String,skel:&mut ProductionTracerSkel,hashmap_pid_to_node:&mut HashMap<i32,String>,
    hashmap_node_to_pid:&mut HashMap<String,i32>,hashmap_links:&mut HashMap<i32,Vec<Link>>,functions: Vec<String>,binary_path:String) -> Result<()> {
    ...
    if Path::new(&nodes_info).exists() {
        ...
        for line in reader.lines() {
            ...
            let node_already_traced = hashmap_node_to_pid.get(&node_name.clone()).is_some();
                ...
                let (tx, rx) = mpsc::channel();
                tx_handles.push(tx);
                start_tracing_container(pid, node_name.clone(),hashmap_links, functions.clone(), 
                binary_path.clone(), skel,rx,&mut join_handles);               
...
pub fn trace_processes_controlled(nodes_info:String,skel:&mut ProductionTracerSkel,hashmap_pid_to_node:&mut HashMap<i32,String>,
    hashmap_node_to_pid:&mut HashMap<String,i32>,hashmap_links:&mut HashMap<i32,Vec<Link>>,functions: Vec<String>,binary_path:String) -> Result<()> {
    ...
    if Path::new(&nodes_info).exists() {
        ...
        for line in reader.lines() {
            ...
            let (tx, rx) = mpsc::channel();
            tx_handles.push(tx);
            start_tracing_process(pid, node_name.clone(),hashmap_links, functions.clone(), binary_path.clone(), skel,rx,&mut join_handles);            
