...
use crate::skel_types::{tracer, SkelEnum};
use anyhow::Result;
...
pub fn run_tracing(
    ...
    let skel_builder = tracer::TracerSkelBuilder::default();
    ...
    let mut skel: tracer::TracerSkel<'static> = open_skel.load()?;
    ...
    let mut skel_enum: SkelEnum<'_, 'static> = SkelEnum::Tracer(&mut skel);
    start_tracing(
        mode,
        functions,
        binary_path,
