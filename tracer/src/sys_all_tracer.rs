...
use crate::skel_types::{sys_all_tracer, SkelEnum};
use anyhow::Result;
...
pub fn run_tracing(
    ...
    let skel_builder = sys_all_tracer::SysAllTracerSkelBuilder::default();
    ...
    let mut skel: sys_all_tracer::SysAllTracerSkel<'static> = open_skel.load()?;
    ...
    let mut skel_enum: SkelEnum<'_, 'static> = SkelEnum::SysAllTracer(&mut skel);
    start_tracing(
        mode,
        functions,
        binary_path,
