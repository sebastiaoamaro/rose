...
def collect_profile(profile_location):
    command = [
        ...
        "mv",
        "/tmp/history.txt",
        profile_location + "faultless_execution.txt",
    ]
    try:
        # Start the process
    ...
    command = [
        ...
        profile_location + "function_stats.txt",
    ]
    try:
        # Start the process
        result = subprocess.run(command, capture_output=True, text=True, check=True)
    ...
    command = [
        "sudo",
        "mv",
        "/tmp/syscall_stats.txt",
        profile_location + "syscall_stats.txt",
    ]
    try:
        # Start the process
        result = subprocess.run(command, capture_output=True, text=True, check=True)
...
def main():
    ...
    if bug_reproduction.binary != "" and bug_reproduction.functions_file == "":
        ...
        for function in function_stats:
            ...
            if ratio > 2:
                print(
                    "Ratio is {}, total calls is {} time_elasped is {}".format(
                        ratio, function[2], elapsed_time
                    )
                )
        ...
        for frequent_function in frequent_functions:
            ...
            delete_function_from_file(
                bug_reproduction.profile + "functions.txt", frequent_function
            )
